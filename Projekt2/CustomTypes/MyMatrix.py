...
from CustomTypes.MyFraction import *
import random
...
class Matrix(object):
    ...
    def makeRandom(cls, m, n, type):
        ...
        for x in range(m):
            if type == 0:
                obj.rows.append([(random.uniform(-65536, 65536 - 1) / 65536) for i in range(obj.n)])
            elif type == 1:
                obj.rows.append([float64(random.uniform(-65536, 65536 - 1) / 65536) for i in range(obj.n)])
            elif type == 2:
                obj.rows.append([float32(random.uniform(-65536, 65536 - 1) / 65536) for i in range(obj.n)])
            elif type == 3:
                obj.rows.append([Fraction(random.uniform(-65536, 65536 - 1), 65536) for i in range(obj.n)])
        return obj

    @classmethod
    def createFromList(cls, listoflists):
        rows = listoflists[:]
